...
Much faster. Both of the above screenshots were from a version of my blog running on my computer, and a VPN was used in the second one to allow me access to the development database (I needed a static IP address to whitelist on the database server, so used my VPN).
...
function loadComments(page) {
	...
	xhr.onload = function() {
		...
		function Comment(props) {
			return html`
				&lt;div data-comment-id="${props.id}" class="${props.type}"&gt
					&lt;table&gt;
						&lt;tr&gt;
							&lt;td&gt;
